...
def multiclass_acc_justy(model,dloader,device):
    ...
    acc = correct_results_sum/y_test.shape[0]*100
    auc = 0
    # auc = roc_auc_score(y_score = prob.cpu(),y_true= y_pred.cpu(), multi_class='ovo')
    return acc, auc
...
def classification_scores(model, dloader, device, task,vision_dset):
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            ...
            #! remain batch size = 1, torch.squeeze() got error
            try:
                y_test = torch.cat([y_test,y_gts.squeeze().float()],dim=0)
            except:
                y_test = torch.cat([y_test,y_gts[0].float()],dim=0)
